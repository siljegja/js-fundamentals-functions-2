...
function timerStatus(remainingMinutes) {
  if (remainingMinutes === 0) return "Phil's cake is ready!"
...
function estimatePrepTime(ingredients, prepTime) {
  if (typeof prepTime === 'undefined') prepTime = 2
...
function calculateQuantities(ingredients, numLayers) {
  const groceries = {}
  groceries.sugar = ingredients.includes('sugar') ? 100 * numLayers : 0
  groceries.eggs = ingredients.includes('eggs') ? 2 * numLayers : 0
  return groceries
}
...
function improveRecipe(receipe, numPortions) {
  for (const ingredient in receipe) {
    receipe[ingredient] *= numPortions
  }
  return receipe
}
